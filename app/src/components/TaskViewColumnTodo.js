...
function TaskViewColumnTodo ( {title, color, status, sprint_id} ) {
    ...
    const getTasks = async e => {
        try {
            const response = await fetch(
                `/api/tasks?status=todo&sprint_id=${selectedSprint}`); //default for fetch() is a get request
...
export default TaskViewColumnTodo;