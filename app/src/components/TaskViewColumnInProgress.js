...
function TaskViewColumnInProgress ( {title, color, sprint_id} ) {

    const [tasks, setTasks] = useState([]);//default value of empty array

    const { selectedSprint } = useContext(SprintContext);

    const getInProgressTasks = async e => {
        try {
            const response = await fetch(
                `/api/tasks?status=inprogress&sprint_id=${selectedSprint}`); //default for fetch() is a get request
...
export default TaskViewColumnInProgress ;